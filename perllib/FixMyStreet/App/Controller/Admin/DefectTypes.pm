...
sub edit : Path : Args(2) {
    ...
    if ($c->req->method eq 'POST') {
        ...
        # $defect_type->external_id( $c->get_param('external_id') );
        foreach my $param ( @{ $c->cobrand->defect_type_extra_fields } ) {
            $defect_type->set_extra_metadata($param, $c->get_param("extra[$param]"));
        }
